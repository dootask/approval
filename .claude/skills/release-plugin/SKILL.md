...
发布靠**推送 tag** 触发 `.github/workflows/release.yml`：构建多架构 Docker 镜像（`kuaifan/dooapprove`）推到 Docker Hub，再打包 `dootask-plugin/` 发布到 DooTask AppStore（appid `approve`）。常规 push / PR 不会触发，光 `git tag` 不 push 也不会。

发布公开且不可逆，**推 tag 前要和用户确认版本号**；常规 git 操作自行判断。

## 三个非踩不可的坑

**1. Tag 不带 `v` 前缀。** 版本号就是 `0.1.8` 这种纯数字。workflow 监听 `tags: '*'`，带 v 也能发，但 Docker tag 会变成 `v0.1.8`，破坏排序和 AppStore 显示。镜像 tag 与插件版本必须一致，所以 tag 名 = 插件版本号。
...
1. 在 `main`、工作区干净、与 `origin/main` 同步；待发布的代码改动先提交推上去。
